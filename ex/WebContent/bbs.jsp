...
<body>
	<div class="container">
	 ...
	 <!-- 변경사항없음 예시로 주석달아봄 -->
	 	...
	 	<div class="container">
	 		<nav>
	 			<form action="bbs?${pageNumber}" method="get">
	 			<fieldset>
	 	...
	 	<div>
	 		...
	 		<a href="bbs" class="btn btn-primary pull-right">게시판</a>
	 		<a href="write.jsp" class="btn btn-primary pull-right">글쓰기</a>
	 		<a href="myWriteList?myPageNumber=${startNumber+1}&f=${param.f}&q=${param.q}" class="btn btn-primary pull-right">나의 글 목록</a>
	 	</div>
	 	</c:if>	
	 </nav>
	</div>
				...
				<c:forEach var="l" items="${list}">
					<tr>
						...
						<td><a href="view?bbsId=${l.bbsId}">${l.bbsTitle}</a>
							<span><a target="_blank" href="myCommentList?bbsId=${l.bbsId}">[${l.countBbsId}]</a></span>
						</td>
						<%-- <td>${l.bbsDate}</td> --%>
...
<%-- 	<c:set var="pagePerContent" value="${pagePerContent}"></c:set> --%>
	...
	<%-- <c:set var="pageNumber" value="${(empty param.pageNumber)? 1 : param.pageNumber}"></c:set> --%>
	<c:set var="startNumber" value="${pageNumber-(pageNumber-1)%5}"></c:set>
	<c:set var="lastNumber" value="${lastNumber}"></c:set>
	<div class="container paginationbox">
			<c:if test="${startNumber>1}">
			<a href="pageNumber=${startNumber-5}" class="btn btn-next btn-primary">이전</a>
			</c:if>
			<c:if test="${startNumber<=1}">
			...
			<ul class="pagination">
				<c:forEach var="i" begin="0" end="4">
					<li><a href="?pageNumber=${startNumber+i}&f=${param.f}&q=${param.q}">${startNumber+i}</a>
				</c:forEach>
			</ul>
			<c:if test="${startNumber+5<lastNumber}">
			<a href="pageNumber=${startNumber+5}" class="btn btn-next btn-primary">다음</a>
			</c:if>
			<c:if test="${startNumber+5>lastNumber}">
	...
	<div style="text-align: cetner">총 페이지 수  : ${totalPage}</div>
	<!-- 애니매이션 담당 JQUERY -->
